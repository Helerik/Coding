...
def f(x):
    ...
    for i in range(1, len(x)):
        retf += x[i]*np.sqrt(1 + ((x[i]-x[i-1])/dx)**2)*dx
    return retf
...
def Grad_f(x):
    ...
    for i in range(len(x)):
        if i == 0:
            grad = -x[i+1]*(x[i+1]-x[i])/(dx*np.sqrt(1 + ((x[i+1]-x[i])/dx)**2))
        elif i == len(x)-1:
            grad = (dx**2 + x[i-1]**2 - 3*x[i-1]*x[i] + 2*x[i]**2)/(dx*np.sqrt(1 + ((x[i]-x[i-1])/dx)**2))
        else:
            grad = dx*np.sqrt(1 + ((x[i]-x[i-1])/dx)**2) + \
                   x[i]*(x[i]-x[i-1])/(dx*np.sqrt(1 + ((x[i]-x[i-1])/dx)**2)) -\
                   x[i+1]*(x[i+1]-x[i])/(dx*np.sqrt(1 + ((x[i+1]-x[i])/dx)**2))
        retgrad.append(grad)
...
def Hess_f(x):
    ...
    for i in range(len(x)):
        if i == 0:
            rethess[i][i] = x[i+1]/(dx*np.sqrt(1 + ((x[i+1]-x[i])/dx)**2)) -\
                            (x[i+1]*(x[i+1] - x[i])**2)/((dx**3)*np.power(1 + ((x[i+1]-x[i])/dx)**2, 3/2))
        elif i == len(x)-1:
            rethess[i][i] = 2*(x[i]-x[i-1])/(dx*np.sqrt(1 + ((x[i]-x[i-1])/dx)**2)) +\
                            x[i]/(dx*np.sqrt(1 + ((x[i]-x[i-1])/dx)**2)) -\
                            (x[i]*(x[i]-x[i-1])**2)/((dx**3)*np.power(1 + ((x[i]-x[i-1])/dx)**2, 3/2))
        else:
            rethess[i][i] = 2*(x[i]-x[i-1])/(dx*np.sqrt(1 + ((x[i]-x[i-1])/dx)**2)) +\
                            x[i]/(dx*np.sqrt(1 + ((x[i]-x[i-1])/dx)**2)) +\
                            x[i+1]/(dx*np.sqrt(1 + ((x[i+1]-x[i])/dx)**2)) -\
                            (x[i]*(x[i]-x[i-1])**2)/((dx**3)*np.power(1 + ((x[i]-x[i-1])/dx)**2, 3/2)) -\
                            (x[i+1]*(x[i+1]-x[i])**2)/((dx**3)*np.power(1 + ((x[i+1]-x[i])/dx)**2, 3/2))
                            
    for i in range(len(x)-1):
        rethess[i][i+1] = -(x[i+1] - x[i])/(dx*np.sqrt(1 + ((x[i+1]-x[i])/dx)**2)) -\
                          x[i+1]/(dx*np.sqrt(1 + ((x[i+1]-x[i])/dx)**2)) +\
                          (x[i+1]*(x[i+1]-x[i])**2)/((dx**3)*np.power(1 + ((x[i+1]-x[i])/dx)**2, 3/2))
        rethess[i+1][i] = rethess[i][i+1]
...
def h(x):
    ...
    h4 = x[len(x)//2]
    for i in range(1, len(x)):
        h3 += np.sqrt(1 + ((x[i]-x[i-1])/dx)**2)*dx
    h3 -= 3
...
def Jacob_h(x):
    ...
    for i in range(len(x)):
        if i == 0:
            grad_h3.append(
                -(x[i+1]-x[i])/(dx*np.sqrt(1 + ((x[i+1]-x[i])/dx)**2))
                )
        elif i == len(x)-1:
            grad_h3.append(
                (x[i] - x[i-1])/(dx*np.sqrt(1 + ((x[i]-x[i-1])/dx)**2))
                )
        else:
            grad_h3.append(
                (x[i] - x[i-1])/(dx*np.sqrt(1 + ((x[i]-x[i-1])/dx)**2)) -\
...
def Hess_h(x):
    ...
    for i in range(len(x)):
        if i == 0:
            hess_h3[i][i] = 1/(dx*np.sqrt(1 + ((x[i+1]-x[i])/dx)**2)) -\
                            ((x[i+1] - x[i])**2)/((dx**3)*np.power(1 + ((x[i+1]-x[i])/dx)**2, 3/2))
        elif i == len(x)-1:
            hess_h3[i][i] = 1/(dx*np.sqrt(1 + ((x[i]-x[i-1])/dx)**2)) -\
                            ((x[i] - x[i-1])**2)/((dx**3)*np.power(1 + ((x[i]-x[i-1])/dx)**2, 3/2))
        else:
            hess_h3[i][i] = 1/(dx*np.sqrt(1 + ((x[i]-x[i-1])/dx)**2)) +\
                            1/(dx*np.sqrt(1 + ((x[i+1]-x[i])/dx)**2)) -\
                            ((x[i] - x[i-1])**2)/((dx**3)*np.power(1 + ((x[i]-x[i-1])/dx)**2, 3/2)) -\
                            ((x[i+1] - x[i])**2)/((dx**3)*np.power(1 + ((x[i+1]-x[i])/dx)**2, 3/2))
    for i in range(len(x)-1):
        hess_h3[i][i+1] = -1/(dx*np.sqrt(1 + ((x[i+1]-x[i])/dx)**2)) +\
                          ((x[i+1] - x[i])**2)/((dx**3)*np.power(1 + ((x[i+1]-x[i])/dx)**2, 3/2))
        hess_h3[i+1][i] = hess_h3[i][i+1]
...
x, lamb = newton_opt(x_init, lamb_init, max_iters = 1000, verbose = 1, do_plot = 0, ro = 0.7, tol = 1e-10)
